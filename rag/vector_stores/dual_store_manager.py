...
class DualStoreManager:
    """
    Manages private and public vector databases using LanceDB.
    """
    ...
    def __init__(self,
        ...
        Args:
            ...
            backend: Storage backend (only 'lancedb' is supported)
        """
        self.logger = logging.getLogger("DualStoreManager")
        self.backend = backend.lower()

        if self.backend != "lancedb":
            raise ValueError(f"Unknown backend: {backend}. Only 'lancedb' is supported.")

        self.logger.info("Using LanceDB backend")
        self.private_store = LanceDBStore(
            db_path=private_db_path,
            collection_prefix="private"
        )
        self.public_store = LanceDBStore(
